...
vec3 GetAlbedo(SurfaceData data)
{
	...
	return mix(data.albedo, vec3(0.f), data.metalness);
}
...
vec3 FresnelSchlick(vec3 f0, vec3 viewDir, vec3 halfDir)
{
	...
	vec3 result = f0 + (vec3(1.f) - f0) * pow(1.f - ClampedDot(viewDir, halfDir), 5.f);
	return result;
}
...
float DistributionGGX(vec3 normal, vec3 halfDir, float roughness)
{
	...
	float result = pow(roughness, 2.f)/(3.14159265358979323846 * pow( pow(ClampedDot(normal, halfDir), 2.f) * (pow(roughness, 2.f) -1.f) + 1.f , 2.f));
	return result;
}
...
float GeometrySchlickGGX(float cosAngle, float roughness)
{
	...
	float result = (2.f * cosAngle) / (cosAngle + sqrt(pow(roughness, 2.f) + (1.f - pow(roughness, 2.f)) * pow(cosAngle,2.f)  )  );
	return result;
}
...
vec3 ComputeDiffuseIndirectLighting(SurfaceData data)
{
	...
	vec3 envSam = SampleEnvironment(data.normal, 1.0f);
	return GetAlbedo(data) * envSam;
}
...
vec3 ComputeSpecularIndirectLighting(SurfaceData data, vec3 viewDir)
{
	...
	vec3 reflection = reflect(data.normal, viewDir);

	// (todo) 08.2: Sample the environment map using the reflection vector, at a specific LOD level
	vec3 envSam = SampleEnvironment(reflection, pow(data.roughness, 0.25f));

	// (todo) 08.6: Add a geometry term to the indirect specular
	float geo = GeometrySmith(data.normal, reflection, viewDir, data.roughness);
...
vec3 CombineIndirectLighting(vec3 diffuse, vec3 specular, SurfaceData data, vec3 viewDir)
{
	...
	//float fresnel = 1.f - dot(data.normal, viewDir);
	vec3 fresnel = FresnelSchlick(data.albedo, viewDir, data.normal);
	

	// (todo) 08.3: Linearly interpolate between the diffuse and specular term, using the fresnel value
	//vec3 energyOut = diffuse * (vec3(1.f) - fresnel) + specular * fresnel;
	vec3 energyOut = mix(diffuse, specular, fresnel);

	//return fresnel * data.ambientOcclusion;
	return energyOut * data.ambientOcclusion;
...
vec3 ComputeDiffuseLighting(SurfaceData data, vec3 lightDir)
{
	...
	return GetAlbedo(data)/3.14159265358979323846f;
}
...
vec3 ComputeSpecularLighting(SurfaceData data, vec3 lightDir, vec3 viewDir)
{
	...
	float distribution = DistributionGGX(data.normal, GetHalfVector(lightDir, viewDir), data.roughness);
	float geometry = GeometrySmith(data.normal, lightDir, viewDir, data.roughness);
	float result = (distribution * geometry) / (4 * (ClampedDot(data.normal, viewDir) * ClampedDot(data.normal, lightDir)) + 0.00001f);
...
vec3 CombineLighting(vec3 diffuse, vec3 specular, SurfaceData data, vec3 lightDir, vec3 viewDir)
{
	...
	vec3 fresnel = FresnelSchlick(data.albedo, viewDir, GetHalfVector(viewDir, lightDir));

	// (todo) 08.3: Linearly interpolate between the diffuse and specular term, using the fresnel value
	vec3 lerped = mix(diffuse, specular, fresnel);

	// (todo) 08.4: Move the incidence factor to affect the combined light value
	float incidence = ClampedDot(data.normal, lightDir);

	//return diffuse + specular;
	return (lerped * incidence);
	//return lerped;
}
