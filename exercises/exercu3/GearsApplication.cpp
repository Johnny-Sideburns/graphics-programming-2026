...
void GearsApplication::Update()
{
    ...
    camera.SetPerspectiveProjectionMatrix(90.f, dimensions, 0.1f, 100.f);
    processInput(GetMainWindow().GetInternalWindow());
...
void GearsApplication::Render()
{
    ...
    m_shaderProgram.SetUniform(m_viewProjUniform, camera.GetViewProjectionMatrix());


    // (todo) 03.1: Draw large gear at the center
    rot = rot + deltaTime;

    glm::mat4 rotationMatrix = glm::rotate(rot, glm::vec3(0.0f, 0.0f, 1.0f));


    glm::mat4 centerGearMatrix(1.0f);
    //centerGearMatrix = glm::rotate(centerGearMatrix, rot, glm::vec3(0.0f, 0.0f, 1.0f));
    centerGearMatrix = centerGearMatrix * rotationMatrix;
    DrawGear(m_largeGear, centerGearMatrix, Color(1.0f, 0.5f, 0.5f));


    // (todo) 03.2: Draw medium gear to the right
    glm::mat4 medGearMatrix(1.0f);
    medGearMatrix = glm::translate(medGearMatrix, glm::vec3(0.75f, 0.0f, 0.0f));
    rotationMatrix = glm::rotate(rot*2,glm::vec3(0.0f,0.0f,1.0f));
    medGearMatrix = medGearMatrix * glm::inverse(rotationMatrix);
    DrawGear(m_mediumGear, medGearMatrix, Color(0.1f, 0.4f, 0.2f));

    // (todo) 03.3: Draw small gear at the top-left corner
    glm::mat4 smallGearMatrix(1.0f);
    smallGearMatrix = glm::translate(smallGearMatrix, glm::vec3(-1.f, 1.f, 0.0f));
    glm::mat4 scaleMatrix = glm::scale(glm::vec3(7.5f, 7.5f, 0.f));
    rotationMatrix = glm::rotate(rot /1.875f +0.045f, glm::vec3(0.f, 0.f, 1.f));
    
    DrawGear(m_smallGear, smallGearMatrix * scaleMatrix * glm::inverse(rotationMatrix), Color(0.1f, 0.0f, 0.8f));

    // (todo) 03.4: Draw small gear linked to the center gear
    glm::mat4 tinyGearMatrix(1.f);
    tinyGearMatrix = glm::translate(tinyGearMatrix, glm::vec3(0.f, 0.2f, 0.0f));

    DrawGear(m_smallGear, centerGearMatrix * tinyGearMatrix * centerGearMatrix, Color(0.5f, 0.5f, 0.0f));
...
void GearsApplication::processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
        position = position + glm::vec3(-1.f, 0.0f, 0.0f) * deltaTime;
        direction = direction + glm::vec3(-1.f, 0.0f, 0.0f) * deltaTime;
        std::cout << "A" << std::endl;
    }
...
void GearsApplication::InitializeShaders()
{
    ...
    wm_matrixUniform = m_shaderProgram.GetUniformLocation("WorldMatrix");
    //std::cout << wm_matrixUniform << std::endl;
...
void GearsApplication::DrawGear(const Mesh& mesh, const glm::mat4& worldMatrix, const Color& color)
{
    ...
    m_shaderProgram.SetUniform(wm_matrixUniform, static_cast<glm::mat4>(worldMatrix));
