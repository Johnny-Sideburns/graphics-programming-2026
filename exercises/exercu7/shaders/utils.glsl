...
vec3 GetImplicitNormal(vec2 normal)
{
	...
	return vec3(normal.x, normal.y, sqrt(1.f -( normal.x * normal.x) - (normal.y * normal.y)));
	//return vec3(0.0f);
}
...
vec3 ReconstructViewPosition(sampler2D depthTexture, vec2 texCoord, mat4 invProjMatrix)
{
	...
	vec4 c  = texture(depthTexture, texCoord);
	float depth = c.x * 2.f - 1.f;
	float x = (texCoord.x * 2.f) - 1.f;
	float y = (texCoord.y * 2.f) - 1.f;
	vec4 pos = invProjMatrix * vec4(x,y, depth, 1.f);
